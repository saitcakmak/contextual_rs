...
def get_beta_hat(X: Tensor, Y_mean: Tensor) -> Tensor:
    ...
    # TODO: This could be memoized since it X won't change through the run.
    x_factors = X.transpose(-2, -1).matmul(X).inverse().matmul(X.transpose(-2, -1))
    return Y_mean.matmul(x_factors.transpose(-2, -1))


def modified_ts(X: Tensor, Y: Tensor, total_budget: int) -> Tensor:
    r"""
    ...
    Returns:
        ...
        allocate to any given arm-context (design point) pair.
    """
...
def modified_ts_plus(X: Tensor, Y: Tensor, total_budget: int) -> Tensor:
    r"""
