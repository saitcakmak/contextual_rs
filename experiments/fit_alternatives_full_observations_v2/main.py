...
def main(
    ...
    for i in range(iterations):
        ...
        eval_context = int(torch.randint(num_contexts, (1,)))
        full_observation = ground_truth.evaluate_all().view(num_arms, num_contexts, -1)
        next_observations = full_observation[:, eval_context]
        train_X = torch.cat([train_X, arm_context_pairs[:, eval_context]], dim=0)
