...
def fit_lcegp(X, Y, emb_dim, fit_tries):
    model = LCEGP(
        X,
        Y,
        categorical_cols=[0],
...
def main(
    ...
    for i in range(iterations):
        ...
        if i % fit_frequency != 0:
            ...
            model = fit_lcegp(X_list[j], Y_list[j], emb_dim, fit_tries)

        # LCEGP
        old_lcegp = model

        next_point = finite_ikg_maximizer(model, arm_set, context_map)
        next_eval = ground_truth.evaluate(next_point[0, 0].long(), next_point[:, 1:])
...
if __name__ == "__main__":
    ...
    if "LCEGP_IKG" in input_dict["labels"]:
        print("IKG already exists in the output. Skipping!")
        quit()
