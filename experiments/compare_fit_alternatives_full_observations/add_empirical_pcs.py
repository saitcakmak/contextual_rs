        ...
        for j in range(num_labels):
            if i % fit_frequency_list[j] != 0:
                model = old_models[j].condition_on_observations(
                    X=train_X[train_data_size + (i - 1) * num_contexts: train_data_size + i * num_contexts].view(-1, context_dim + 1),
                    Y=train_Y[train_data_size + (i - 1) * num_contexts: train_data_size + i * num_contexts].view(-1, 1),
                )
            else:
                model = LCEGP(
