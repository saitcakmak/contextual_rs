...
def fix_output(
    ...
    **kwargs,
    ...
    assert labels == ["5-1", "10-1", "25-1", "10-5", "25-5", "25-10"]
        ...
        for j in range(num_labels):
            if i % fit_frequency_list[j] != 0:
                model = old_models[j].condition_on_observations(
                    X=X_list[j][train_data_size + i - 1].view(1, context_dim + 1),
                    Y=Y_list[j][train_data_size + i - 1].view(1, 1),
                )
            else:
                model = LCEGP(
                    X_list[j][: train_data_size + i],
