...
from botorch.test_functions import Branin, Hartmann, SyntheticTestFunction
...
class SineTestFunction(SyntheticTestFunction):
    dim = 2
...
class GroundTruthModel:
    def __init__(
        ...
        if function is None:
            ...
            self.arm_map = torch.rand(num_arms, 1).to(context_map)
            if function == "branin":
                self.function = Branin(negate=True)
            elif function == "hartmann":
                self.function = Hartmann(negate=True)
            elif function == "sine":
                self.function = SineTestFunction()
            else:
                raise NotImplementedError
            scale_x = unnormalize(torch.rand(1000, self.dim), self.function.bounds).to(
                context_map
            )
...
def main(
    ...
    if label not in ["Li", "Gao"] or mode == "-reeval":
        X = (
            torch.cat(
                [
    ...
    for i in range(existing_iterations, iterations):
        ...
        if "LCEGP" in label or mode == "-reeval":
            # the explicit indices are for -reeval
        ...
        if mode == "-reeval":
            next_X = reeval_X[i]
            if "Li" in label or "Gao" in label:
                next_point = torch.cat(
                    [next_X[:1].view(-1), context_map[next_X[1:].long()].view(-1)]
                ).view(1, -1)
            else:
                next_point = next_X.view(1, -1)
            next_eval = reeval_Y[i].view(-1, 1)
        elif "LCEGP" in label or "ST" in label:
            ...
            if "IKG" in label:
                ...
                next_arm, next_context = contextual_complete_ei(
                    model, arm_set, context_map, randomize_ties
                )
                next_point = torch.cat(
        ...
        if label not in ["LCEGP_IKG", "ST_IKG"] and mode != "-reeval":
